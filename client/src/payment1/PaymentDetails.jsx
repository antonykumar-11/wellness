...
const PaymentVoucher = () => {
  ...
  const [updatePayment, { error }] = useUpdatePurchaseMutation();
  ...
  const { data } = useCheckVoucherNumbersQuery(purchaseData, {
    skip: !purchaseData?.thisPurchase || !purchaseData?.status, // Skip query if either value is missing
  });
  ...
  useEffect(() => {
    setPurchaseData((prevData) => ({
      ...
      paymentMethod: purchaseData.description,
    }));
  }, [selectedVoucherIds]);
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      // Step 1: Modify debitLedgers logic before creating the payment
      const updatedPurchaseData = {
        ...purchaseData,
        debitLedgers: [
          { ledgerId: "", ledgerName: "", amount: 0 }, // Index 0 with value 0
          purchaseData.debitLedgers?.[1] || {
            ledgerId: "",
            ledgerName: "",
            amount: purchaseData.amount,
          }, // Index 1 with value from purchaseData
        ],
        creditLedgers: purchaseData.creditLedgers || [
          { ledgerId: "", ledgerName: "", amount: "" },
        ],
      };

      // Step 2: Create the payment using the updated purchaseData
      const paymentResult = await createPayment(updatedPurchaseData).unwrap();
  ...
  console.log("selected debitNote", setDebitNote);
  ...
  useEffect(() => {
    if (selectedVoucherId) {
      ...
      if (selectedVoucher) {
        const { _id, __v, debitLedgers, creditLedgers, ...filteredVoucher } =
          selectedVoucher; // Extract transactionId, debitLedgers, creditLedgers
        ...
        setPurchaseData((prevData) => ({
          ...
          selectedOption: prevData.selectedOption, // Preserve the selected
          debitLedgers: debitLedgers || [
            { ledgerId: "", ledgerName: "", amount: "" },
          ], // Use the extracted debitLedgers
          creditLedgers: creditLedgers || [
