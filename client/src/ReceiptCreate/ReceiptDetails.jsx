...
const PaymentVoucher = () => {
  ...
  console.log("Receipt dataData", purchaseData);
  ...
  const [updatePayment, { error }] = useUpdateSalesVoucherMutation();
  ...
  const [createPayment] = useCreateReceiptVoucherMutation();
  ...
  const handleSave = async (e) => {
    ...
    try {
      // Step 1: Create the payment using purchaseData
      const paymentResult = await createPayment(purchaseData).unwrap();
      ...
      console.error("Failed to create purchase or payment:", error);
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      const payload = {
        transactionId: debitNote.transactionId, // Assuming transactionId is available in your component
        updatedPurchase: {
          ...purchaseData, // Spread the current purchaseData
          debitLedgers: debitNote.debitLedgers, // Use debit details from debitNote state
          creditLedgers: debitNote.creditLedgers, // Use credit details from debitNote state
          voucherType: "Sales Voucher",
        },
      };

      // Step 1: Submit the updated purchase data with ledgers
      const createPurchaseResult = await updatePayment(payload).unwrap();

      // Check if the voucherType is "Purchase Voucher"
      if (createPurchaseResult.voucherType === "Sales Voucher") {
        // Step 2: Create the payment using purchaseData
        const paymentResult = await createPayment(purchaseData).unwrap();
    ...
    <div className="p-6">
      ...
      <div className="lg:hidden mt-10">
        {purchaseData.items.map((item, index) => (
          ...
          >
            {/* Stock Name Input */}
            <div className="font-bold mb-2 mt-4  ">Stock Name</div>
            <input
              type="text"
      ...
      <div className="my-10">
        ...
        <div className="my-10 flex">
          {/* Conditionally render the Save or Update button */}
          {selectedVoucherId ? (
            <button
              type="button"
