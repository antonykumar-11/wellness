...
const ContraVoucher = () => {
  ...
  } = useGetContraVoucherByIdQuery(transactionId || "", {
    skip: !transactionId,
  });
  const [updateJournalVoucher] = useUpdateContraVoucherMutation();
  const [deleteJournalVoucher] = useDeleteContraVoucherMutation();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      refetchVoucherData();
      navigate("/reports/contrareports"); // Redirect after update
    } catch (error) {
  ...
  const handleDelete = async () => {
    if (transactionId) {
      try {
        ...
        navigate("/reports/contrareports"); // Redirect after delete
      } catch (error) {
      ...
      <main className="flex-grow flex justify-center items-center p-4">
        <div className={containerClass}>
          <h2 className="text-3xl font-bold mb-6 text-center">
            {transactionId ? "Edit contra" : "Create contra"}
          </h2>
