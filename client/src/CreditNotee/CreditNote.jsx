...
import { useCreateCreditNoteMutation } from "../store/api/CreditNoteApi";
...
const CreditNote = () => {
  ...
  const { data: purchaseVouchers = [] } = useGetSalesVouchersQuery();
  ...
  const handleLedgerSelect = (type, option) => {
    ...
    if (type === "credit") {
      ...
      setSearchTermCredit(option.name); // Update search term for credit ledger

      setPurchaseData((prevData) => {
        const updatedCreditLedgers = [...prevData.creditLedgers];

        // Update or add the ledger based on its index (index 0 for credit)
      ...
      setSearchTermDebit(option.name); // Update search term for debit ledger

      setPurchaseData((prevData) => {
        const updatedDebitLedgers = [...prevData.debitLedgers];

        // Update or add the ledger based on its index (index 1 for debit)
