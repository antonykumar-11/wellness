...
const DebitNotePreviews = () => {
  ...
  console.log("debitNote Sate", purchaseData);
  ...
  } = useGetDebitNoteByIdQuery(transactionId || "", {
  ...
  const [updatePayment] = useUpdateDebitNoteMutation();
  const [deletePayment] = useDeleteDebitNoteMutation();
  useEffect(() => {
    refetch();
  }, [refetch]);
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      if (updateResult) {
        ...
        navigate("/reports/debitNotereports");
  ...
  useEffect(() => {
    if (paymentVoucher) {
      ...
      setPurchaseData((prevData) => ({
        ...
        ...filteredVoucher,
        voucherNumber: filteredVoucher.voucherNumber || "", // Ensure voucherNumber is defined or provide a fallback
        voucherType: "Debit Note", // Corrected the type to match your needs
        ...
        purchasedBy: purchasedBy?._id || "", // Extract ID from paymentVoucher
        purchasedTo: purchasedTo?._id || "", // Extract ID from paymentVoucher
      }));
  ...
  useEffect(() => {
    if (paymentVoucher) {
      ...
      setPurchaseData((prevData) => ({
        ...prevData, // Preserve previous data
        ...filteredVoucher,
        voucherNumber: filteredVoucher.voucherNumber || "", // Ensure voucher1 is defined or provide a fallback
        voucherType: "Debit Note",
        creditDueDate: new Date(filteredVoucher.creditDueDate)
          .toISOString()
          .split("T")[0],
...
export default DebitNotePreviews;
