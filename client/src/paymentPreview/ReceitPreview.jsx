...
const ReceiptPreview = () => {
  ...
  const {
    ...
    refetch: updateRefetch, // Alias `refetch` as `refetchVoucherData`
  ...
  const [deletePayment] = useDeleteReceiptVoucherMutation();
  const [updatePayment] = useUpdateReceiptVoucherMutation();

  useEffect(() => {
    // Assuming you want to refetch after updatePayment has been called

    refetch(); // Refetch data when the component mounts or dependencies change
    updateRefetch(); // Call updateRefetch if needed
  }, [updateRefetch, refetch]); // Dependencies can be kept as needed
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      const payload = {
        transactionId, // Ensure transactionId is available
        updatedPurchase: purchaseData, // Spread the current purchaseData
      };
            ...
            <>
              ...
              <div className="relative flex-1 min-w-[200px]">
                ...
                <input
                  ...
                  id="taxLedger"
                  value={
                    debitNote.creditLedgers[1]?.ledgerName // If index 1 exists, use it for tax
              ...
              <div className="relative flex-1 min-w-[200px]">
                ...
                <input
                  ...
                  id="creditLedger"
                  value={
                    debitNote.creditLedgers[1]?.ledgerName || // If index 1 exists, use it for credit ledger
                    debitNote.creditLedgers[0]?.ledgerName || // If only index 0 exists, use it as credit ledger
