...
import { useCheckVoucherNumberQuery } from "../store/api/PaymentApi";
...
import { useUpdatePurchaseMutation } from "../store/api/PurchaseApi";
...
const Paymenrview = () => {
  ...
  console.log("paymenrview", purchaseData);
  ...
  const [updatePayment, { error }] = useUpdatePaymentMutation();
  const {
  ...
  } = useGetPaymentByIdQuery(transactionId || "", {
    skip: !transactionId, // Skip the query if `transactionId` is falsy
  });
  console.log("paymentVoucherpaymentVoucherpaymentVoucher", paymentVoucher);
  useEffect(() => {
    refetchVoucherData();
  }, [refetchVoucherData]);
  const dropdownRefs = {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      const payload = {
        transactionId, // Use the transactionId from debitNote or any relevant state
      ...
      const updateResult = await updatePayment(payload).unwrap(); // Assuming updatePayment is the API call
  ...
  useEffect(() => {
    if (voucherCheck && voucherCheck.length > 0) {
      ...
      const existingVoucherNumbers = voucherCheck.map(
        (voucher) => parseInt(voucher.voucherNumber, 10) // Ensure numbers are parsed correctly
      );
      ...
      // Set the voucher number in both voucher1 state and purchaseData
      setVoucher1(newVoucherNumber); // Set as string if needed
      setPurchaseData((prevData) => ({
        ...prevData, // Spread the previous state to retain existing fields
        voucherNumber: newVoucherNumber, // Update voucherNumber
      }));
    }
  }, [voucherCheck]);
  ...
  //       if (selectedVoucher) {
  //         // Extract stockGroupNames from items
  ...
  // console.log("selectedVoucherselectedVoucherselectedVoucher",selectedVoucher)
  //         setPurchaseData((prevData) => ({
  //           ...filteredVoucher,
  //           voucherNumber: voucher1,
  //           purchasedBy: here set id,
  //           purchasedTo:here set id ,
  //           voucherType: "Purchase Voucher", // Automatically set voucherType to "Payment Voucher"
  //           // VoucherNumberTrue: filteredVoucher.voucherNumber,
  ...
  //         }));
  //         setSearchTermPurchasedBy(
  //           filteredVoucher.purchasedBy?.companyName || "dfefv"
  //         );
  //         setSearchTermPurchasedTo(
  //           filteredVoucher.purchasedTo?.companyName || "fefrde"
  //         );
  ...
  //         setVoucherKuttan(filteredVoucher.voucherNumber || "");
  //       }
  //     }
  //   }, [selectedVoucherId, purchaseVouchers]);
  console.log("purchaseVoucherspurchaseVouchers", purchaseVouchers);
  useEffect(() => {
    if (selectedVoucherId) {
      ...
      if (selectedVoucher) {
        ...
        console.log("selectedVoucher", selectedVoucher.purchasedBy);
        ...
        // Update searchTerms with the stockGroupNames
        setSearchTerms((prev) => ({
          ...prev,
          ...stockGroupNamesObject, // Add stockGroupNames to the searchTerms state
        }));
  ...
  useEffect(() => {
    if (paymentVoucher && !selectedVoucherId) {
      // This ensures it only runs if no voucher is selected
      ...
      setPurchaseData((prevData) => ({
        ...filteredVoucher,
        voucherNumber: filteredVoucher.voucherNumber,
        voucherType: "Payment Voucher", // Setting the correct type
        creditDueDate: new Date(filteredVoucher.creditDueDate)
          .toISOString()
          .split("T")[0],
        transactionDate: new Date(filteredVoucher.transactionDate)
  ...
  }, [paymentVoucher, selectedVoucherId]); // Dependent on paymentVoucher and selectedVoucherId
  ...
  useEffect(() => {
    if (paymentVoucher) {
      if (paymentVoucher) {
        console.log(
          "paymentVoucherpaymentVoucherpaymentVoucher",
          paymentVoucher
        );
        const {
          _id,
          ...
          purchasedTo,
          ...filteredVoucher
        } = paymentVoucher; // Extract transactionId
  ...
  }, [selectedVoucherId, purchaseVouchers, paymentVoucher]);
        ...
        <div className="flex flex-wrap gap-4 justify-center">
          ...
          <div className="relative flex-1 min-w-[200px]">
            ...
            <input
              ...
              className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline cursor-not-allowed dark:bg-gray-700 dark:border-gray-600 dark:text-gray-200"
            />
          </div>
