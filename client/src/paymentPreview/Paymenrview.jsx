...
const Paymenrview = () => {
  ...
  const [updateJournalVoucher] = useUpdatePaymentMutation();
  const [deleteJournalVoucher] = useDeletePaymentMutation();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      refetchVoucherData();
      navigate("/reports/paymentreport"); // Redirect after update
    } catch (error) {
  ...
  const handleDelete = async () => {
    if (transactionId) {
      try {
        ...
        navigate("/reports/paymentreport"); // Redirect after delete
      } catch (error) {
        console.error("Failed to delete Payment voucher:", error);
      }
    }
  };
      ...
      <main className="flex-grow flex justify-center items-center p-4">
        <div className={containerClass}>
          <h2 className="text-3xl font-bold mb-6 text-center">
            {transactionId ? "Edit Payment Voucher" : "Create Payment Voucher"}
          </h2>
