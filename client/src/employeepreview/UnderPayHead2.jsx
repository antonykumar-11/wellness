...
function UnderPayHead1({ options, hello, manu }) {
  const [search, setSearch] = useState(manu || ""); // Initialize search with manu
  ...
  // Effect to update search if manu changes
  useEffect(() => {
    setSearch(manu || ""); // Update search when manu prop changes
  }, [manu]);
  ...
  const handleSelectOption = (option) => {
    hello(option);
    setOpen(false);
    setSearch(option.name); // Update search to the selected option name
  };
    ...
    <div className="relative w-full">
      <div id={`Select-${id}`} className="relative w-full">
        <input
          ...
          value={search} // Use search state directly
...
UnderPayHead1.propTypes = {
  ...
  manu: PropTypes.string, // Added prop type for manu
};

UnderPayHead1.defaultProps = {
  options: [],
  hello: () => {},
  manu: "", // Added default prop for manu
};
